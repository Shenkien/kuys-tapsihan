...
const STATUS_COPY: Record<string, { label: string; description: string }> = {
  ...
  CONFIRMED: { label: "Confirmed", description: "The kitchen has confirmed your order." },
  ...
  COMPLETED: { label: "Completed", description: "Enjoy your meal!" },
  CANCELLED: { label: "Cancelled", description: "This order was cancelled. Please ask staff for help." },
};
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3 sm:px-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {channel === "KIOSK" ? "Self-Service Kiosk" : tableNumber ? `Table ${tableNumber} — Order to Go` : "Order to Go"}
          </p>
        </div>
        {showCartButton && (
          <button
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                {hasAddOns && (
                  ...
                  >
                    + Add extras (Extra Rice, Egg, Drinks…)
                  </button>
                )}
              </div>
            </div>
...
function ComboGrid({ combos, cart }: { combos: PublicComboMeal[]; cart: ReturnType<typeof useCart> }) {
  if (combos.length === 0) {
    return (
      <div className="mx-auto max-w-5xl px-6 py-16 text-center text-muted-foreground">
        No combo meals available right now.
      </div>
    );
  }
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {addOns.length > 0 && (
            <div className="mt-4">
              <p className="mb-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Add-ons</p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5">
        <div>
          <label className="mb-1 block text-sm font-medium" htmlFor="customer-name">
            {channel === "KIOSK" ? "Name for order pickup call (optional)" : "Name (optional)"}
          </label>
          <input
            id="customer-name"
            value={customerName}
        ...
        <div>
          <p className="mb-2 block text-sm font-medium">Payment method</p>
          ...
          <p className="mt-1.5 text-xs text-muted-foreground">
            {paymentMethod === "CASH"
              ...
              : "Payment is settled at the counter — this just lets staff know what to expect."}
          </p>
        </div>
...
function ConfirmationView({
  ...
  return (
    <div className="mx-auto max-w-lg px-6 py-16 text-center">
      ...
      <p className="mt-1 text-muted-foreground">Show this number when you pick up your order.</p>
      ...
      <div className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-sm text-muted-foreground">Total: {formatCurrency(order.totalAmount)}</p>
