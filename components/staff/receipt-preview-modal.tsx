...
import { formatReceiptText, type ReceiptData } from "@/lib/receipt";

export function ReceiptPreviewModal({ receipt, onClose }: { receipt: ReceiptData; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="border-t border-border bg-secondary/40 px-4 py-3 text-xs text-muted-foreground">
          ...
          this exact slip prints automatically instead of showing here.
        </div>
      </div>
    </div>
  );
