...
async function main() {
  ...
  // --- Sales Management: business settings (tax/service charge/discount) ---
  // Lazily created on first use too (see lib/settings.ts), but seeding it
  // here means the values show up in Admin > Settings right away instead
  // of only appearing after the first order/payment.
  const existingSettings = await prisma.businessSettings.findFirst();
  if (!existingSettings) {
    await prisma.businessSettings.create({
      data: {
        vatRate: 12,
        vatInclusive: true, // menu prices above already include VAT
        serviceChargeRate: 0, // small tapsihan, no service charge by default
        seniorPwdDiscountRate: 20, // RA 9994 / RA 10754
        receiptPrefix: "OR",
      },
    });
  }

  // --- Sample supplier (for Purchase Orders demo) ---
  await prisma.supplier.upsert({
    where: { name: "Bicutan Wet Market Suppliers" },
    update: {},
    create: {
      name: "Bicutan Wet Market Suppliers",
      contactPerson: "Aling Nena",
      phone: "0917-000-0000",
      email: "orders@bicutansuppliers.example",
      address: "Lower Bicutan Public Market, Taguig",
    },
  });
