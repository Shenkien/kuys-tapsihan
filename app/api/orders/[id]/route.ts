...
import { publishKitchenEvent, KITCHEN_EVENTS, publishOrderStatusEvent } from "@/lib/pusher-server";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Customer Notification Function — pushes the same status change to
    // this order's own tracking channel, so a customer with /track/[id]
    // open gets the update instantly (and can vibrate/chime on READY)
    // instead of waiting on the tracking page's 5s poll fallback.
    await publishOrderStatusEvent(id, { status: serialized.status });
